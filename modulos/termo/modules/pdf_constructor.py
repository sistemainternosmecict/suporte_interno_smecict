...
class PdfConstructor:
    def __init__(self, first_paragraph: str, title: str, data: dict)-> bool:
        self.numero = random.randint(1000000, 9999999)
        self.criar_qr(self.numero)
        self.title = title
        self.base_color = Color(0.0, 0.50196, 0.50196)
        self.template_pdf = "modulos/termo/template.pdf"
        self.temp_pdf = "modulos/termo/temp.pdf"
        ...
        self.export_pdf = os.path.join(self.export_folder, f"{self.first_name.lower()}_{self.numero}.pdf")
        ...
        self.gerado = {"path":f"{self.first_name.lower()}_{self.numero}.pdf", "gerado":self.merge_pdf(self.template_pdf, self.temp_pdf, self.export_pdf)}
    ...
    def criar_qr(self, numero:int):
        url = f"http://sistec/api/termo/{numero}"
